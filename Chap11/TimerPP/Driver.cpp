...
NTSTATUS DispatchClose (
		...
		IN PIRP				pIrp			) {
#if DBG>=1
	DbgPrint("TIMERPP: DispatchClose\n");
#endif
...
NTSTATUS DispatchWrite (
		...
		IN PIRP				pIrp			) {
#if DBG>=1
	DbgPrint("TIMERPP: Write Operation requested (DispatchWrite)\n");
#endif
...
#if DBG>=1
	DbgPrint("TIMERPP: Read Operation requested (DispatchRead)\n");
#endif
...
//--
 BOOLEAN TransmitByte( 
	...
	// Now send it...
#if DBG>=2
	DbgPrint("TIMERPP: TransmitByte: Sending 0x%02X to port %X\n",
	...
	pDevExt->deviceBuffer[pDevExt->xferCount++] =
		readByte;
#if DBG>=2
	DbgPrint("TIMERPP: TransmitByte read character: 0x%03X\n",
				readByte);
#endif

	// Start the polling timer
#if DBG>=2
	DbgPrint("TIMERPP: TransmitByte starting timer\n");
#endif
	KeSetTimer(
...
#if DBG>=1
	DbgPrint("TIMERPP: Start I/O Operation\n");
#endif
...
#if DBG>=1
	DbgPrint("TIMERPP: StartIO: Transmitting first byte of %d\n", pDevExt->deviceBufferSize);
#endif
...
#if DBG>=1
	DbgPrint("TIMERPP: PollingTimerDpc Fired\n");
#endif
...
#if DBG>=1
	DbgPrint("TIMERPP: PollingTimerDpc Completing IRP\n");
#endif
		PIRP pIrp = pDevObj->CurrentIrp;
